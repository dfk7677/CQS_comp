// === CompetitiveConquest8v8_4flag_nohq.ts ===
...
const capturePoints: CapturePoints = {
    ...
    204: { status: 0, capturer: 0, onPoint: [0,0], symbol: "D", fade: mod.Pi() }
};
...
function ChangeTickets() {
    ...
    if (teamcps[0] == 4) {
        ...
        for (let i = 0; i < n; i++) {
            const player = mod.ValueInArray(players, i);
            const team = mod.GetTeam(player);
            if (mod.Equals(team, team1) && mod.GetSoldierState(player, mod.SoldierStateBool.IsAlive)) {
                return;
            }
...
async function initializeGamePhase() {
    ...
    if (gamePhase == 0) {
        ...
        mod.SetCapturePointCapturingTime(mod.GetCapturePoint(204), CAPTURE_TIME);
        ...
        mod.SetCapturePointNeutralizationTime(mod.GetCapturePoint(204), NEUTRALIZE_TIME);
        ...
        mod.SetMaxCaptureMultiplier(mod.GetCapturePoint(204), MAX_CAPTURE_MUTLIPLIER);
        addCountdownUI();
    } else if (gamePhase == 2) {
        // Live phase logic
...
export function OnCapturePointCaptured(eventCapturePoint: mod.CapturePoint): void {
    ...
    if (id === 201 && mod.Equals(mod.GetCurrentOwnerTeam(eventCapturePoint), team1)) {
        for (let i = 711; i < 716; i++) {
            const vehicleSpawner = mod.GetVehicleSpawner(i)
            if (vehicleSpawner) {
                mod.SetVehicleSpawnerAutoSpawn(vehicleSpawner, true);
    ...
    if (id === 204 && mod.Equals(mod.GetCurrentOwnerTeam(eventCapturePoint), team2)) {
        for (let i = 741; i < 746; i++) {
            const vehicleSpawner = mod.GetVehicleSpawner(i)
            if (vehicleSpawner) {
                mod.SetVehicleSpawnerAutoSpawn(vehicleSpawner, true);
